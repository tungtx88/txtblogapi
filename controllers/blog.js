...
exports.create = (req, res) => {
    ...
    form.parse(req, (err, fields, files) => {
        ...
        if (!categories || categories === 0) {
            return res.status(400).json({
                error: 'You should choose at least one category'
            })
        }

        if (!tags || tags === 0) {
            return res.status(400).json({
                error: 'You should choose at least one tag'
            })
        }
...
exports.read = (req, res) => {
    ...
    Blog.findOne({ slug })
        ...
        .populate('tags', '_id name slug')
        .populate('postedBy', '_id name username')
        .select('_id title body slug excerpt mtitle mdesc categories tags postedBy createdAt updatedAt')
        .exec((err, data) => {
            if (err) {
                return res.status(400).json(
                    {
...
exports.listRelated = (req, res) => {
    ...
    Blog.find({ _id: { $ne: _id }, categories: { $in: categories } })
        ...
        .select('title body slug excerpt postedBy createdAt updatedAt')
        .exec((err, blogs) => {
            if (err) {
                return res.status(400).json({
...
exports.prevBlog = (req, res) => {
    const { _id } = req.body.blog
    Blog.findOne({ _id: { $lt: _id } })
        .sort({ _id: -1 })
...
exports.nextBlog = (req, res) => {
    const { _id } = req.body.blog
    Blog.findOne({ _id: { $gt: _id } })
        .sort({ _id: -1 })
        .select('title slug updatedAt')
        .exec(
            (err, blog) => {
